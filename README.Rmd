...
df <- sfheaders::sf_to_df(silicate::minimal_mesh)
calc_area(df)
sf::st_area(sf::st_set_crs(silicate::minimal_mesh, NA))


df <- sfheaders::sf_to_df(silicate::inlandwaters)
calc_area(df) %>% summarize(area = sum(area)) %>% pull(area)
